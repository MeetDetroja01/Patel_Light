...
export default function ImportExportPage() {
  ...
  async function handleImport() {
    ...
    if (res.ok) {
      ...
      toast({ title: `✅ Imported ${data.inserted} bills!`, description: data.skipped ? `${data.skipped} rows skipped.` : undefined });
    } else {
  ...
  function exportCSV(bills: BillRow[]) {
    ...
    const rows = bills.map((b) =>
      COLUMNS.map((c) => {
        const val = String(b[c.key as keyof BillRow] ?? "");
        return `"${val.replace(/"/g, '""')}"`;
      }).join(",")
    );
    const csv = [header, ...rows].join("\n");
    download(new Blob([csv], { type: "text/csv;charset=utf-8;" }), "patel-light-bills.csv");
  }
  ...
  async function exportPDF(bills: BillRow[]) {
    ...
    doc.text(`Generated: ${new Date().toLocaleDateString("en-IN")}   Total: ${bills.length} bills`, 14, 20);
    ...
    autoTable(doc, {
      ...
      columnStyles: { 9: { halign: "right" } }, // amount right-aligned
    });
      ...
      <div className="animate-fade-in space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 ml-auto">Downloads all bills</span>
          </div>
          <div className="p-6">
            <p className="text-sm text-slate-500 mb-5">
              Export your complete bills database in your preferred format.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {([
                { fmt: "csv"   as ExportFormat, icon: "📄", label: "CSV",   desc: "Universal spreadsheet format",  color: "#10b981" },
                { fmt: "excel" as ExportFormat, icon: "📊", label: "Excel", desc: "Microsoft Excel (.xlsx)",        color: "#1d6f42" },
                { fmt: "pdf"   as ExportFormat, icon: "📋", label: "PDF",   desc: "Printable landscape report",    color: "#Fd0304" },
              ]).map(({ fmt, icon, label, desc, color }) => (
                <button
                  key={fmt}
                  onClick={() => handleExport(fmt)}
                  disabled={exporting !== null}
                  className="flex flex-col items-center gap-3 p-5 rounded-2xl border-2 border-dashed transition-all hover:border-solid disabled:opacity-50 disabled:cursor-not-allowed group"
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 ml-auto">CSV or Excel file</span>
          </div>
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-blue-800 mb-1">Use the correct column format</p>
                <p className="text-xs text-blue-600 mb-2">
                  Your file must have these column headers (case-insensitive):
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {COLUMNS.map((c) => (
                    <span key={c.key} className="text-[11px] px-2 py-0.5 bg-blue-100 text-blue-700 rounded font-mono font-medium">
                      {c.label}
                    </span>
                  ))}
                </div>
                <button onClick={downloadTemplate}
                  className="mt-3 text-xs font-semibold text-blue-700 hover:text-blue-900 underline underline-offset-2 transition-colors">
            ...
            >
              ...
              {fileName ? (
                ...
                <>
                  <p className="font-semibold text-slate-600 text-sm">Click to select file</p>
            ...
            {preview.length > 0 && (
              <div>
                ...
                >
                  {importing ? "Importing…" : `⬆️ Import ${preview.length}+ Bills`}
                </button>
              </div>
            )}
              ...
              <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl p-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-600 mt-0.5">
                    {importResult.inserted} bills imported
...
async function downloadTemplate() {
  ...
  const ws = XLSX.utils.aoa_to_sheet([
    ["Date", "Bill Number", "Customer Name", "Mobile Number", "Shop Name", "State", "City", "Transport Name", "Items Sent", "Total Amount"],
